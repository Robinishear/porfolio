        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="relative group">
            ...
            <div className="absolute left-0 top-full pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 translate-y-4 group-hover:translate-y-0 z-[110]">
              <div className="bg-zinc-900 border border-white/10 w-64 p-5 rounded-[24px] shadow-2xl backdrop-blur-2xl bg-opacity-95">
                <p className="text-cyan-400 text-[10px] font-bold tracking-[0.2em] mb-4 text-center uppercase">My Documents</p>
             <div className="flex flex-col gap-3">
                  <a href="#" className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-cyan-500/10 hover:border-cyan-500/30 transition-all group/item">
                    <FaDownload className="text-cyan-500 group-hover/item:scale-110 transition-transform" />
                    <span className="text-xs font-semibold text-white/80">Download CV</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-blue-500/10 hover:border-blue-500/30 transition-all group/item">
                    <FaFileAlt className="text-blue-500 group-hover/item:scale-110 transition-transform" />
                    <span className="text-xs font-semibold text-white/80">Download Resume</span>
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="relative group">
            ...
            <div className={`
              ...
              ${isMobileContactOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible translate-y-4"}
            ...
            `}>
              <div className="bg-zinc-900 border border-white/10 w-[280px] sm:w-[350px] md:w-[450px] p-4 md:p-6 rounded-[24px] md:rounded-[32px] shadow-2xl backdrop-blur-2xl bg-opacity-95">
                <div className="flex justify-between items-center mb-4 lg:block">
                    <p className="text-white/50 text-[10px] md:text-xs font-bold tracking-[0.2em] text-center w-full">GET IN TOUCH</p>
                    {/* মোবাইল ক্লোজ বাটন */}
                    <button onClick={() => setIsMobileContactOpen(false)} className="lg:hidden text-white/50 hover:text-white">
                        <FaTimes />
                    </button>
                </div>
        ...
        <div className="flex flex-col items-center justify-start pt-10 space-y-6 overflow-y-auto h-full pb-32">
          {navLinks.map((link) => (
            <Link key={link.name} href={link.href} onClick={() => setMobileMenuOpen(false)}>
              <span className="text-2xl font-bold bg-blue-800/50 text-white/80 hover:text-cyan-400 transition-colors uppercase tracking-widest">{link.name}</span>
            </Link>
          ))}
